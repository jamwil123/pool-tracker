    ...
    <BrowserRouter>
      <Container maxW="6xl" py={8}>
        <Box className="app-header" as="header" p={6} borderRadius="lg" boxShadow="md" bg="white">
          <Box>
            <Heading size="lg">{APP_TITLE} {(import.meta as any).env?.DEV ? (
              <Badge ml={2} colorScheme="green">DEV</Badge>
            ) : null}</Heading>
            <Text mt={1}>{TEAM_NAME}</Text>
            <Text mt={1}>Signed in as {user.email} · Role: {profile?.role || 'player'}</Text>
            <Flex align="center" justify="space-between" gap={4} mt={3} wrap="wrap">
              <HStack gap={4} as="nav">
                <RouterLink to="/">Home</RouterLink>
                <RouterLink to="/games">Matches</RouterLink>
                {profile && isManagerRole(profile.role) ? <RouterLink to="/dashboard">Captain's Dashboard</RouterLink> : null}
              </HStack>
              <HStack gap={3}>
